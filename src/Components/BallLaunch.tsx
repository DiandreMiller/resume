...
const BallLaunch = () => {
    ...
    // const [bounceEffect, setBounceEffect] = useState<number>(0.5);
    ...
    const bounceThreshold: number = 1.5;
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setYPosition((previousY) => {
                ...
                if (newY >= floorY) {
                    newY = floorY;
                    setVelocityY((previousVelocityY) => 
                        Math.abs(previousVelocityY) > bounceThreshold
                        ? -previousVelocityY * bounceEffect
                        : 0
                    ); // Bounce effect
...
//     const [velocityY, setVelocityY] = useState<number>(0);
//     const [rotation, setRotation] = useState<number>(0);
//     const [bounceEffect, setBounceEffect] = useState<number>(-0.8); // Bounce effect as state
...
//         // Set velocity based on distance (basic calculation)
//         const launchFactor: number = 0.1; // Adjust the launch factor for better height
//         setVelocityX(distanceX * launchFactor);
//         setVelocityY(distanceY * launchFactor);
//     };
...
//                     // Optionally update bounceEffect here based on game conditions
//                     // For example, you could decrease bounceEffect over time:
//                     // setBounceEffect(prev => prev * 0.95);
//                 } else {
//                     setVelocityY(velocityY + gravity);
//                 }
//                 return newY;
